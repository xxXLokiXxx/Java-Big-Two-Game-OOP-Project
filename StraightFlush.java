/** 
 * A subclass of the Hand class and is used to model straight flush used in a 
 * Big Two card game.
 *
 * @author Loki
 *
...
public class StraightFlush extends Hand {
	...
	/** 
	 * a constructor for building a straight flush 
	...
	public boolean isValid() {
		...
		if (this.size() != 5) {
			return false;     // check for hand size of 5
		}
		else if ((card0.getSuit() == card1.getSuit()) && (card1.getSuit() == card2.getSuit()) && (card2.getSuit() == card3.getSuit()) && (card3.getSuit() == card4.getSuit())) {
			if ((card0.rank == card1.rank -1) && (card1.rank == card2.rank -1) && (card2.rank == card3.rank -1) && (card3.rank == card4.rank -1)) {
				return true;            // check for same suit and consecutive rank
			}
			else {
				return false;
			}
	 ...
	 *  @return the type (StraightFlush) of hand in string
	 */
